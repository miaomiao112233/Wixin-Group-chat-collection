...
- **Word 五模块排版**：统计概览表格、话题分节摘要、活跃人物+关键发言引用、重要事项待办、文件清单（含"未下载"标注）
- **文件智能归档**：解析 `totallen` 字节大小 + 下载时间双重判据，正确区分同名不同内容文件；微信没下完的会自动等下轮重试
- **增量去重游标**：每群独立 sort_seq 游标，崩溃重启幂等续跑；只归档启用日及之后的消息，不回溯历史
- **简洁 PySide6 界面**：群卡片网格、状态开关、设置对话框（API Key 含小眼睛、自动归档开关、微信目录手动选择）、托盘常驻
